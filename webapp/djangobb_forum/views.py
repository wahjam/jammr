...
logger = logging.getLogger('djangobb_forum.views')


def index(request, full=True):
    users_cached = cache.get('djangobb_users_online', {})
    users_online = users_cached and User.objects.filter(id__in=list(users_cached.keys())) or []
...
def reject_spam_topic(request, form, poll_form):
    """
    deactivate and log out first-time users who create a poll and use their
    username as the topic.
    """
    # Spam posts usually include a poll for some reason
    if not poll_form.has_data():
        return False

    # The topic name is usually the username
    topic_name = form.cleaned_data['name'].strip().lower()
    username = request.user.get_username().strip().lower()
    if topic_name != username:
        return False

    # Only consider the first post that a user makes
    if request.user.forum_profile.post_count > 0:
        return False

    # Deactivate spammer account
    logger.error('Deactivating suspected forum spammer account "{}" for message {}'.format(request.user.get_username(), repr(form.cleaned_data['body'])))
...
def add_topic(request, forum_id):
    ...
    if request.method == 'POST':
        ...
        if all_valid:
            if reject_spam_topic(request, form, poll_form):
                return HttpResponseRedirect(reverse('djangobb:index'))
